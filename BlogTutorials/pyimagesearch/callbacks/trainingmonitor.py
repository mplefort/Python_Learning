...
class TrainingMonitor(BaseLogger):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if len(self.H["loss"]) > 1:
            ...
            plt.ylabel("Accuracy")
            plt.legend()
            plt.savefig(self.figPath + "Acc.png")
            plt.close()

            plt.figure()
            plt.plot(N, self.H["loss"], 'bo', label='Training loss')
            plt.plot(N, self.H["val_loss"], 'b', label='Validation loss')
            plt.title("Training and Validation and Loss")
            plt.xlabel("Epoch #")
            plt.ylabel("loss")
            plt.legend()
            plt.savefig(self.figPath + "Loss.png")
            plt.close()