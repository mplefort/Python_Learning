...
class Field(object):
    ...
    def addDrunk(self, drunk, loc):
        # drunk has to be immutable so they are hashable for a dictionary lookup.
        if drunk in self.drunks or not isinstance(loc, Location):
            raise ValueError('Duplicate drunk or not a Location')
        else:
...
class Drunk(object):
    # base class, only meant to be inherited
...
# Both drunk classes are immutable, nothing inside this class is changed.
...
# Sanity check, bug for numSteps ILO of numTrails
...
# random.seed(0)
# numSteps = (10,100,1000,10000, 50000)
